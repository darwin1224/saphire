...
func (vm *VM) Run() error {
	...
	for vm.currentFrame().ip < len(vm.currentFrame().Instructions())-1 {
		...
		case code.OpGetGlobal:
			globalIndex := code.ReadUint16(ins[ip+1:])
			vm.currentFrame().ip += 2
			err := vm.push(vm.constants[globalIndex])
			if err != nil {
				return err
			}
		case code.OpReturn:
...
func (vm *VM) executeBinaryOperation(op code.Opcode) error {
	...
	switch {
	case leftType == object.NUMBER_OBJ && rightType == object.NUMBER_OBJ:
...
func (vm *VM) executeComparison(op code.Opcode) error {
	...
	switch op {
	case code.OpEqual:
		return vm.push(boolToBooleanObject(right == left))
...
func (vm *VM) executeNumberComparison(op code.Opcode, left, right object.Object) error {
	...
	switch op {
	case code.OpEqual:
		return vm.push(boolToBooleanObject(rightValue == leftValue))
	case code.OpNotEqual:
		return vm.push(boolToBooleanObject(rightValue != leftValue))
	case code.OpGreaterThan:
		return vm.push(boolToBooleanObject(leftValue > rightValue))
	default:
...
func (vm *VM) buildHash(startIndex, endIndex int) (object.Object, error) {
	...
	return &object.Hash{Pairs: hashedPair}, nil
}
...
func (vm *VM) executeArrayIndex(array, index object.Object) error {
	...
	return vm.push(arrayObject.Elements[int(i)])
}
